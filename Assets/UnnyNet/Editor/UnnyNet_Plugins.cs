...
using UnnyNet.Editor;

public class UnnyNet_Plugins
{
    private const string PLUGINS_LOCAL_FOLDER = "Assets/UnnyNet/";
    private const string PLUGINS_ADDRESS_LOCAL = "Assets/UnnyNet/Editor/plugins.json";
    private const string PLUGINS_ADDRESS_ORIGINAL = "Assets/UnnyNet/Editor/unnynet_plugins.json";
    private const string PLUGINS_ADDRESS_REMOTE = "https://dictionaries-unnynet.fra1.cdn.digitaloceanspaces.com/config/unnynet_plugins.json";
    
    private const string CODE_GENERATION_PLUGIN = "UnnyNet";
    private const string CODE_GENERATION_FILE = "Assets/UnnyNet/Scripts/UnnyNetMain.cs";
...
#pragma warning restore 649
        ...
        IEnumerator InstallAllFiles()
        {
            ...
            for (int i = 0; i < Download.Length; i++)
            {
                ...
                Utils.CheckAndCreateDirectoryForFile(fullPath);
...
#pragma warning restore 649
    ...
    public UnnyNet_Plugins(EditorWindow parent)
    {
    ...
    private void SaveLocalFile()
    {
        ...
        Utils.CheckAndCreateDirectoryForFile(PLUGINS_ADDRESS_LOCAL);
    ...
    private void RenderNormalEditorUpdate()
    {
        ...
        GUILayout.BeginHorizontal(EditorStyles.helpBox);
        GUILayout.Label("New version of UnnyNet plugin is now available! Hooray!");
        GUI.color = Color.white;
    ...
    private void RenderForceEditorUpdate()
    {
        GUI.color = Color.red;
        GUILayout.BeginHorizontal(EditorStyles.helpBox);
        GUILayout.Label("Please update UnnyNet plugin to the newest version!");
        GUI.color = Color.white;
        if (GUILayout.Button("Update v" + _pluginsRemote.EditorInfo.Version.ToString(), LAYOUT_BUTTON))
            Application.OpenURL(_pluginsRemote.EditorInfo.DownloadUrl);
        GUILayout.EndHorizontal();
